...
const QuestionsPage = ({ user }) => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:3003/api/${personAge}/${caseFileId}`); 
        if (!response.ok) {
  ...
  const handleSubmit = (event) => {
    ...
    const isCorrect = selectedAnswer === (currentQuestion.y_correct_answer || currentQuestion.o_correct_answer); // Check if the selected answer is correct
    ...
    if (currentQuestionIndex === questions.length - 1) {
      ...
      navigate(`/countries/${countryId}/case_files/${caseFileId}/questions/results`, {
